...
Shader "Custom/Canvas/Backplate" {

Properties {

    [Header(Round Rect)]
        _MainTex_("MainTex", 2D) = "white" {}
    ...
    // Added to avoid UnityUI warnings.
...
CBUFFER_END
    ...
    VertexOutput vert(VertexInput vertInput)
    {
        ...
        o.uv = UV;
        o.texuv = TRANSFORM_TEX(vertInput.uv0, _MainTex_);
    ...
    half4 frag(VertexOutput fragInput) : SV_Target
    {
        ...
        bool To_sRGB = false;
        float4 test= tex2D(_MainTex_,fragInput.texuv);
        result = Out_Color*test;
        return result;
    }
